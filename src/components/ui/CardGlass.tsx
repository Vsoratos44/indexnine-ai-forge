...
const base =
  "rounded-3xl border shadow-glass backdrop-blur-xl transition-all duration-300";
const variants: Record<NonNullable<CardGlassProps["variant"]>, string> = {
  ...
  light:
    "bg-glass-bg-light border-glass-border hover:shadow-glass-lg hover:border-brand-primary/30",
};
...
>(({ className, ...props }, ref) => (
  <h3
    ref={ref}
    className={cn("heading-h3 text-foreground", className)}
    {...props}
  />
));
CardGlassTitle.displayName = "CardGlassTitle";

const CardGlassMeta = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement>
>(({ className, ...props }, ref) => (
  <div
...
const CardGlassBody = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement>
>(({ className, ...props }, ref) => (
  <div
